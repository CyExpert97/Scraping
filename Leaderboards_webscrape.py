...
class LOLChampionStatsScraper:
    ...
    def get_stats(self):
        ...
        rank_2 = self.driver_ranks.find_element_by_xpath('/html/body/div[3]/div[3]/div[3]/div/div/div/div[1]/ul/li[2]/a').get_attribute('href')
        ex = {'Url of player': rank_2}
        df = df.append(ex, ignore_index=True)
        rank_3 = self.driver_ranks.find_element_by_xpath('/html/body/div[3]/div[3]/div[3]/div/div/div/div[1]/ul/li[3]/a').get_attribute('href')
        ex = {'Url of player': rank_3}
        df = df.append(ex, ignore_index=True)
        rank_4 = self.driver_ranks.find_element_by_xpath('/html/body/div[3]/div[3]/div[3]/div/div/div/div[1]/ul/li[4]/a').get_attribute('href')
        ex = {'Url of player': rank_4}
        df = df.append(ex, ignore_index=True)
        rank_5 = self.driver_ranks.find_element_by_xpath('/html/body/div[3]/div[3]/div[3]/div/div/div/div[1]/ul/li[5]/a').get_attribute('href')
        ex = {'Url of player': rank_5}
        df = df.append(ex, ignore_index=True)
        # Main loop of getting rest of top 95 players from table
        items = self.driver_ranks.find_elements_by_xpath("/html/body/div[3]/div[3]/div[3]/div/div/div/table/tbody/tr/td[2]/a")
        for item in items:
